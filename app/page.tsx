...
async function getLatestArticles(): Promise<ArticlePreview[]> {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("content_items")
      .select("id,title,slug,excerpt,primary_keyword,updated_at")
      .eq("status", "published")
      .order("published_at", { ascending: false })
      .limit(6);

    if (error) {
      console.error("Homepage content query failed:", error.message);
      return [];
    }
    return (data || []) as ArticlePreview[];
  } catch (error) {
    console.error("Homepage content backend unavailable:", error);
    return [];
  }
}
...
export default async function Home() {
  ...
  return <><Header/><main className="container">
    ...
    <section style={{marginTop:70}}><h2 className="section-title">Latest guides</h2><div className="article-list">{articles.length?articles.map(a=><Link className="article-row" href={`/articles/${a.slug}`} key={a.id}><div><h3>{a.title}</h3><p>{a.excerpt}</p></div><span>→</span></Link>):<div className="panel">No published guides yet. The public site is online and ready for content.</div>}</div></section>
  </main></>;
}
