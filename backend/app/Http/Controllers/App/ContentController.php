...
class ContentController extends Controller {
    public function index(Request $request, WorkspaceContext $ctx){ $workspace=$ctx->workspace($request); $site=$ctx->site($request); $items=ContentItem::where('workspace_id',$workspace->id)->where('site_id',$site->id)->latest('updated_at')->paginate(25); return view('app.content-index',compact('workspace','site','items')); }
    ...
    public function edit(Request $request, ContentItem $content, WorkspaceContext $ctx){ $workspace=$ctx->workspace($request); $this->guard($content,$workspace->id); return view('app.content-edit',['workspace'=>$workspace,'site'=>$content->site,'item'=>$content,'versions'=>$content->versions()->latest()->limit(15)->get()]); }
    ...
    public function improve(Request $request, ContentItem $content, WorkspaceContext $ctx, PlanService $plans, ContentEngine $engine, AuditService $audit){
        ...
        try{$plans->assertCanUse($workspace,'ai_improvements'); $engine->improve($content,$data['reason'],false); $plans->record($workspace,'ai_improvements',1,$content->site,$request->user(),['content_id'=>$content->id]); $audit->log($request,$workspace,'content.improved',$content,['reason'=>$data['reason']]);}catch(RuntimeException $e){return back()->withErrors(['reason'=>$e->getMessage()]);}
        return back()->with('success','AI improvement created; the previous version is preserved.');
    }
